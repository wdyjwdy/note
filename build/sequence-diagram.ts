...
function render(data: Data) {
	...
	const [width, height] = [gridLenX * config.actorWidth - 50, gridLenY * 50]
...
function renderSignal(
	...
	const x1 = gridX1 * config.actorWidth + 50
	const x2 = gridX2 * config.actorWidth + 50
...
function renderActor(text: string, gridX: number, gridY: number) {
	const x = gridX * config.actorWidth
	const y = gridY * 50
	const cx = x + 50
	const cy = y + 25
...
function renderActorLine(gridX: number, gridLenY: number) {
	const x1 = gridX * config.actorWidth + 50
	const x2 = x1
	const y1 = 50
